...
#     outpath = r'D:\cloudromove\result\TMnet\TM-kpl-256-3_color'

#     intpath = r'D:\cloudromove\result\TMnet\TM-kpl-256-3'
    # outpath = r'./kpl/TMnet/KPL-1_color'
    # intpath = r'./kpl/TMnet/KPL-1'
    # outpath = r'/root/autodl-tmp/test_result/tc_山地全时相/TMnet/TC-3_color'
    # outpath = r'/root/autodl-tmp/test_result/tc_山地全时相/TMnet/TC-3_color_jia'
    # intpath = r'/root/autodl-tmp/test_result/tc_山地全时相/TMnet/TC-3'
    # outpath = r'../kpl_农田/TMnet/KPL-1_l3_color'
    outp = r'../test_result/kpl_delTrans/TMnet'
    length = 75
    
    outpath = os.path.join(outp,'KPL-1_color_jia')
    intpath = os.path.join(outp,'KPL-1')
    # outpath = r'../kpl_hl/TMnet/KPL-1_color_jia'
    # intpath =r'../kpl_hl/TMnet/KPL-1'
    # outpath = r'../tc_my/TMnet/KPL-1_l1_color'
    # outpath = r'../tc_my/TMnet/KPL-1_l3_color_jia'
    # intpath =r'../tc_my/TMnet/KPL-1_l3'
    pathlist = ['cloud', 'gt', 'result']
    for pathname in pathlist:
        ...
        for idx, name in enumerate(listdir):
            ...
            data4 = run.read_img(intpath + '/' + pathname + '/' + pathname + '_' +
                                 str(int(n[1][:-4]) + length * 0) + name[-4:])  # Blue

            out = outpath_ + '/' + name

            data = np.array((data1, data2, data3), dtype=data1.dtype)  # 按序将3个波段像元值放入
            # data = np.array((data2, data3, data4), dtype=data2.dtype)  # 按序将3个波段像元值放入
            # data = np.array((data4, data3, data2), dtype=data2.dtype)  # 按序将3个波段像元值放入
            data = truncated_linear_stretch(data, truncated_value=0.8, max_out=255, min_out=0)
            # 第三步
            run.write_img(out, data)
            
    # outpath = r'../kpl_hl/TMnet/KPL-2_color_jia'
    # intpath =r'../kpl_hl/TMnet/KPL-2'
    outpath = os.path.join(outp,'KPL-2_color_jia')
    intpath = os.path.join(outp,'KPL-2')
    # outpath = r'../tc_my/TMnet/KPL-1_l1_color'
    # outpath = r'../tc_my/TMnet/KPL-1_l3_color_jia'
    # intpath =r'../tc_my/TMnet/KPL-1_l3'
    pathlist = ['cloud', 'gt', 'result']
    for pathname in pathlist:
        ...
        for idx, name in enumerate(listdir):
            ...
            data = np.array((data1, data2, data3), dtype=data1.dtype)  # 按序将3个波段像元值放入
            # data = np.array((data4, data2, data3), dtype=data2.dtype)  # 按序将3个波段像元值放入
            data = truncated_linear_stretch(data, truncated_value=0.8, max_out=255, min_out=0)
            # 第三步
            run.write_img(out, data)
    ...
    # outpath = r'../kpl_hl/TMnet/KPL-2_color_jia'
    # intpath =r'../kpl_hl/TMnet/KPL-2'
    outpath = os.path.join(outp,'KPL-3_color_jia')
    intpath = os.path.join(outp,'KPL-3')
    # outpath = r'../tc_my/TMnet/KPL-1_l1_color'
    # outpath = r'../tc_my/TMnet/KPL-1_l3_color_jia'
    # intpath =r'../tc_my/TMnet/KPL-1_l3'
    pathlist = ['cloud', 'gt', 'result']
    for pathname in pathlist:
        ...
        for idx, name in enumerate(listdir):
            ...
            data = np.array((data1, data2, data3), dtype=data1.dtype)  # 按序将3个波段像元值放入
            # data = np.array((data3, data2, data4), dtype=data2.dtype)  # 按序将3个波段像元值放入
            data = truncated_linear_stretch(data, truncated_value=0.8, max_out=255, min_out=0)
            # 第三步
            run.write_img(out, data)