...
class tDataset(data.Dataset):
    ...
    def __init__(self, config, transform=None):
        ...
        if not os.path.exists(test_list_file) or os.path.getsize(test_list_file) == 0:
            files = os.listdir(os.path.join(config.datasets_dir, config.years[0]))
            np.savetxt(os.path.join(config.datasets_dir, config.train_list), np.array(files), fmt='%s')
            np.savetxt(os.path.join(config.datasets_dir, config.test_list), np.array(files), fmt='%s')
        self.imlist = np.loadtxt(test_list_file, str)

    def __getitem__(self, index):
        cloud = cv2.imread(os.path.join(self.config.test_dir, self.config.years[0], str(self.imlist[index])),
                           cv2.IMREAD_GRAYSCALE).astype(np.float32)
        x1 = cv2.imread(os.path.join(self.config.test_dir, self.config.years[1], str(self.imlist[index])),
                        cv2.IMREAD_GRAYSCALE).astype(np.float32)
        x2 = cv2.imread(os.path.join(self.config.test_dir, self.config.years[2], str(self.imlist[index])),
                        cv2.IMREAD_GRAYSCALE).astype(np.float32)
        x3 = cv2.imread(os.path.join(self.config.test_dir, self.config.years[3], str(self.imlist[index])),
                        cv2.IMREAD_GRAYSCALE).astype(np.float32)
        x4 = cv2.imread(os.path.join(self.config.test_dir, self.config.years[4], str(self.imlist[index])),
                        cv2.IMREAD_GRAYSCALE).astype(np.float32)
        ...
        return cloud, temp, m, str(self.imlist[index])[:-4] + '.bmp'
