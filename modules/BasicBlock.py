...
#                                                                                                   comp_B_feats)
#         l1 = self.l1loss(real_B_, fake_B_)
...
class fftLoss(nn.Module):
    ...
    def forward(self, x, y):
        ...
        diff = torch.fft.fft2(x.to('cuda:0')) - torch.fft.fft2(y.to('cuda:0')) #fft傅里叶变换
        loss = torch.mean(abs(diff)) #abs()取绝对值；mean()取均值
        
        #loss = self.l1loss(torch.fft.fft2(x.to('cuda:0')), torch.fft.fft2(y.to('cuda:0'))) #fft傅里叶变换
...
class Vgg19(torch.nn.Module):
    def __init__(self, requires_grad=False):
        ...
        #调整vgg19的输入通道为1，并将其卷积层分为5个部分，输入图像为[1,1,256,256]

        # vgg_pretrained_features[0] = nn.Conv2d(1, 64, kernel_size=3, padding=1) #调整vgg19的输入通道为1
        vgg_pretrained_features[0] = nn.Conv2d(4, 64, kernel_size=3, padding=1) #调整vgg19的输入通道为4
